...
export default function CombustivelPage({ user, outro, colors, refreshKey, triggerRefresh }) {
  ...
  const [showStatsHistorico, setShowStatsHistorico] = useState(false)
  ...
  function calcularStatsMes(moto, carro) {
    const hoje = new Date()
    const mesAtual = hoje.getMonth()
    const anoAtual = hoje.getFullYear()
    const inicioMes = new Date(anoAtual, mesAtual, 1)
    ...
    const filtrarMes = (arr) => arr.filter(d => {
      const data = new Date(d.createdAt)
      return data >= inicioMes && data <= fimMes
    })

    const motoMes = filtrarMes(moto)
    const carroMes = filtrarMes(carro)
    const todosMes = [...motoMes, ...carroMes]
    ...
    const gastosSusanna = todosMes.filter(d => d.buyer === 'Susanna').reduce((sum, d) => sum + d.total_value, 0)
    const gastosPietrah = todosMes.filter(d => d.buyer === 'Pietrah').reduce((sum, d) => sum + d.total_value, 0)
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-base-700/50 backdrop-blur-sm border border-white/5 rounded-3xl p-5">
        <SectionTitle>Estatísticas - {mesAtual}</SectionTitle>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-white text-sm font-semibold">{statsMes.contadores?.Susanna?.moto || 0}x</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-white/50 text-xs">Carro</span>
                <span className="text-white text-sm font-semibold">{statsMes.contadores?.Susanna?.carro || 0}x</span>
              </div>
              <div className="flex justify-between items-center pt-2 border-t border-white/5">
                <span className="text-white/50 text-xs">Total gasto</span>
                <span className="text-white text-sm font-semibold">{fmt(statsMes.gastosSusanna || 0)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-white/50 text-xs">Moto</span>
                <span className="text-white text-sm font-semibold">{statsMes.contadores?.Pietrah?.moto || 0}x</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-white/50 text-xs">Carro</span>
                <span className="text-white text-sm font-semibold">{statsMes.contadores?.Pietrah?.carro || 0}x</span>
              </div>
              <div className="flex justify-between items-center pt-2 border-t border-white/5">
                <span className="text-white/50 text-xs">Total gasto</span>
                <span className="text-white text-sm font-semibold">{fmt(statsMes.gastosPietrah || 0)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-white/10">
          <div className="flex justify-between items-center">
            ...
            <span className="text-white font-bold text-lg">{fmt(statsMes.totalGastos || 0)}</span>
          </div>
        </div>

        {/* Gráfico de Gastos por Pessoa - Mês */}
        {(statsMes.gastosSusanna > 0 || statsMes.gastosPietrah > 0) && (() => {
          const dadosGasto = [
            { name: 'Susanna', valor: statsMes.gastosSusanna || 0, fill: coresSu.hex },
            { name: 'Pietrah', valor: statsMes.gastosPietrah || 0, fill: coresPi.hex },
          ]
          const diff = (statsMes.gastosSusanna || 0) - (statsMes.gastosPietrah || 0)
      ...
      {/* Estatísticas - Todo Histórico (Expander) */}
      <div className="bg-base-700/50 backdrop-blur-sm border border-white/5 rounded-3xl overflow-hidden">
        <button
          onClick={() => setShowStatsHistorico(!showStatsHistorico)}
          className="w-full px-5 py-4 flex items-center justify-between hover:bg-white/5 transition-colors"
        >
          <span className="text-white font-medium text-sm">Estatísticas - Todo Histórico</span>
          <ChevronDown size={16} className={`text-white/40 transition-transform ${showStatsHistorico ? 'rotate-180' : ''}`} />
        </button>
