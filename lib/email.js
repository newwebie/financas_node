...
// ========================================
// 3. EMAIL DE NOTIFICAÇÃO DE COMPRA
// ========================================
export async function enviarEmailNotificacaoCompra(quemComprou, item, categoria, valorTotal, tipoCompra, descricao) {
  try {
    const transporter = getTransporter()

    const destinatarioNome = quemComprou === 'Susanna' ? 'Pietrah' : 'Susanna'
    const destinatarioEmail = getEmailDestinatario(destinatarioNome)
    ...
    const isDividido = tipoCompra === 'Dividido (me deve metade)'
    const valorDevido = isDividido ? valorTotal / 2 : valorTotal
    const valorDevidoFmt = valorDevido.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })

    const mensagemDivertida = isDividido
      ? 'Olha aqui o que você também comprou! 🛒'
      : `Olha o que ${quemComprou} comprou pra você com o seu dinheiro! 💸🎁`

    const labelDevido = isDividido ? 'Sua metade' : 'Valor total'
    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; background-color: #0d0d14; padding: 20px;">
      <div style="max-width: 400px; margin: 0 auto; background: linear-gradient(135deg, #1a1a2e 0%, #12121f 100%); border-radius: 16px; padding: 24px; border: 1px solid rgba(255,255,255,0.1);">
        ...
        <div style="background: rgba(110, 231, 183, 0.1); padding: 16px; border-radius: 12px; text-align: center; margin: 0 0 12px 0; border: 1px solid rgba(110, 231, 183, 0.2);">
          <span style="font-size: 11px; color: rgba(255,255,255,0.4); text-transform: uppercase; letter-spacing: 1px;">🏷️ ${categoria}</span><br>
          <span style="font-size: 20px; color: #6ee7b7; font-weight: 700;">${item}</span>
          ${descricao ? `<br><span style="font-size: 12px; color: rgba(255,255,255,0.4);">${descricao}</span>` : ''}
        </div>

        <div style="background: rgba(244, 114, 182, 0.1); padding: 16px; border-radius: 12px; text-align: center; margin: 0 0 12px 0; border: 1px solid rgba(244, 114, 182, 0.2);">
          <span style="font-size: 11px; color: rgba(255,255,255,0.4); text-transform: uppercase; letter-spacing: 1px;">💸 ${labelDevido}</span><br>
          <span style="font-size: 28px; color: #f472b6; font-weight: 700;">${valorDevidoFmt}</span>
          ${isDividido ? `<br><span style="font-size: 11px; color: rgba(255,255,255,0.3);">Total da compra: ${valorFmt}</span>` : ''}
        </div>

        <div style="background: rgba(253, 186, 116, 0.1); padding: 12px; border-radius: 12px; text-align: center;">
          <p style="font-size: 13px; color: #fdba74; margin: 0;">
            😉 Comprado por <strong>${quemComprou}</strong>
          </p>
        </div>
    ...
    await transporter.sendMail({
      from: `"${process.env.SMTP_FROM_NAME}" <${process.env.SMTP_FROM_EMAIL}>`,
      to: destinatarioEmail,
      subject: `🛍️ Notificação de compra: ${item}`,
      html,
    })
