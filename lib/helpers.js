...
export function matchLugar(descricao, lugares) {
  ...
  const desc = descricao.replace(/\s+/g, ' ').toLowerCase()
  const descLimpa = limparDescricao(descricao).replace(/\s+/g, ' ').toLowerCase()
  return lugares.find(lugar =>
    lugar.keywords?.some(kw => {
      if (!kw) return false
      const k = kw.toLowerCase()
      // Prefixo: keyword terminando em * faz match pelo início da descrição
      // Ex: "juros saldo utiliz ate limite*" bate em "juros saldo utiliz ate limite periodo: 21/02 a 20/03/26"
      if (k.endsWith('*')) {
        const prefix = k.slice(0, -1)
        return desc.startsWith(prefix) || descLimpa.startsWith(prefix)
      }
      // Match exato (substring)
      if (desc.includes(k) || descLimpa.includes(k)) return true
      // Tolerância a truncamento de nomes (PDF corta nomes longos no fim)
      // Ex: keyword "thiago cesar da costa neves", desc tem "thiago cesar da costa nev"
      if (k.length > 12 && (k.startsWith(descLimpa) || k.startsWith(desc))) return true
      return false
    })
  ) || null
}
